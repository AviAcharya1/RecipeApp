...
const LikedProducts = () => {
  ...
  const fetchLikedProducts = async () => {
    try {
      ...
      const response = await fetch("http://localhost:8001/auth/likedRecipes"
        // "https://recipe-app.onrender.com/auth/likedRecipes"
      );
  ...
  const handleRemoveItem = async (recipeId) => {
    try {
      if (
        ...
        const response = await fetch(`http://localhost:8001/auth/removeLiked/${recipeId}`,
          // `https://recipe-app.onrender.com/auth/removeLiked/${recipeId}`,
          {
            method: "DELETE",
          }
        );
